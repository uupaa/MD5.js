...
function MD5(source) { // @arg String: source string
                       // @ret String: md5 hash string. lower case
                       // @help: MD5
//{@assert
    _if(typeof source !== "string", "invalid MD5(source)");
//}@assert

    var md5 = MD5_encode(Binary.BinaryStringToUintArray(source)).map(function(value) {
...
// --- implement -------------------------------------------
function MD5_encode(source) { // @arg ByteArray: [byte, ...]
                              // @ret ByteArray: [...]
                              // @help: MD5.encode
//{@assert
    _if(!Array.isArray(source), "invalid MD5.encode(source)");
//}@assert
...
function _MD5(source) { // @arg ByteArray:
                        // @ret ByteArray:
                        // @desc: calc MD5
        ...
        for (j = 0; j < 64; ++j) {
            n = j < 16 ? (b & c) | (~b & d) // ff - Round 1
              ...
              : j < 48 ?  b ^ c ^ d         // hh - Round 3
                       :  c ^ (b | ~d);     // ii - Round 4
            n += a + word[_MD5_X[j]] + _MD5_A[j];

            ra = b + ((n << _MD5_S[j]) | (n >>> (32 - _MD5_S[j])));
            rb = b;
            rc = c;
            // --- rotate ---
            a = d;
